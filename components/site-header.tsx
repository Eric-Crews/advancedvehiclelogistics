...
export function SiteHeader({ onNavigate, admin = false, active = 'request' }: { onNavigate?: (page: SiteView) => void; admin?: boolean; active?: SiteView }) {
  ...
  function go(page: SiteView) { setOpen(false); if (page === 'board') { router.push('/drivers'); return; } if (onNavigate) onNavigate(page); else router.push(page === 'request' ? '/' : page === 'review' ? '/desk' : `/?view=${page}`); }
